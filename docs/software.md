...
!!! tip "重复安装问题"
    很多用户都需要使用同一个软件，例如 anaconda。我们已经在集群的公共目录上安装了软件，由于存储空间有限，请不要再重复安装同一款软件，而是使用我们安装的软件。因此，使用前请务必认真阅读我们提供的在线手册。

## 软件加载方法

!!! note "什么是 Linux 环境变量"
    在Linux系统下使用软件需要了解“环境变量”这个概念。不同的软件会被安装在不同的目录下，环境变量决定了Linux系统将到哪个目录中寻找安装好的软件。

    我们去学校图书馆去借阅一本《哈利波特7》，在图书馆门口大声喊出“我要看哈利波特！”，并不能帮你找到这本书。你需要在图书馆的检索系统中先查到这本书所在的书架号码，然后到对应的书架上查找对应的书。

    Linux 环境变量是一个类似的检索系统，用户在命令行或脚本中使用软件时，Linux 系统会从环境变量的所有路径下，从前到后寻找用户输入的软件名，然后到该目录下，调用该软件，执行程序。一些常用的软件，如 `ssh` 已经被默认添加在系统的环境变量中。一些用户安装的软件一般要将该软件所在目录手动加载到系统的环境变量。
